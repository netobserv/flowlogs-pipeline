...
func udnKey(label, ip string) string {
	return label + "~" + ip
}
...
func (m *UDNHandler) BuildKeys(flow config.GenericMap, rule *api.K8sRule) []string {
	var keys []string
...
func (m *UDNHandler) GetPodUniqueKeys(ctx context.Context, dynClient *dynamic.DynamicClient, pod *v1.Pod) ([]string, map[string]string, error) {
	// Example:
	...
	if statusAnnotationJSON, ok := pod.Annotations[ovnAnnotation]; ok {
		...
		if err := json.Unmarshal([]byte(statusAnnotationJSON), &annot); err != nil {
			return nil, nil, fmt.Errorf("failed to index from OVN annotation, cannot read annotation %s: %w", ovnAnnotation, err)
		}
