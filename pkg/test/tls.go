...
func CreateClientCerts(t *testing.T) (string, string, string, func()) {
	...
	require.NoError(t, err)
	uc, err, cleanupUC := DumpToTemp(clientCert)
	require.NoError(t, err)
	uk, err, cleanupUK := DumpToTemp(clientKey)
	require.NoError(t, err)
...
func CreateAllCerts(t *testing.T) (string, string, string, string, string, func()) {
	...
	uk, err, cleanupUK := DumpToTemp(clientKey)
	require.NoError(t, err)
	sc, err, cleanupSC := DumpToTemp(serverCert)
	require.NoError(t, err)
	sk, err, cleanupSK := DumpToTemp(serverKey)
	require.NoError(t, err)
